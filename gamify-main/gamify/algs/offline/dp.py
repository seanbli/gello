...
from gamify.networks.base import ActorPolicy
...
class DiffusionPolicy(OffPolicyAlgorithm):
    ...
    def _predict(self, batch: Dict, return_obs_encoding=False) -> torch.Tensor:
        if isinstance(batch["obs"], torch.Tensor):
            obs_value = batch["obs"]
        elif isinstance(batch["obs"], dict):
            obs_value = next(iter(batch["obs"].values()))
            if isinstance(obs_value, dict):
                obs_value = next(iter(obs_value.values()))
        B = obs_value.shape[0]
    ...
    def get_obs_encoding(self, batch: Any, is_batched: bool = False, **kwargs) -> Any:
        is_np = not utils.contains_tensors(batch)
        if not is_batched:
            # Unsqeeuze everything
            batch = utils.unsqueeze(batch, 0)
        batch = self.format_batch(batch)
        enc = self._predict(batch, return_obs_encoding=True, **kwargs)
        if not is_batched:
            enc = utils.get_from_batch(enc, 0)
        if is_np:
            enc = utils.to_np(enc)
        return enc
