...
def load_data(
    path: str,
    override_keys: Optional[List[Tuple[str]]],
    ...
    with open(path, "rb") as f:
        ...
        if include_keys is not None:
            keys_to_load = include_keys
        elif exclude_keys is not None:
            keys_to_load = data_file.keys()

        data = {}
        override_keys = {new_key: old_key for new_key, old_key in override_keys}
        for k in keys_to_load:
            if k in override_keys.keys():
                new_key = k
                old_key = override_keys[k]
            else:
                new_key = k
                old_key = k
            data[new_key] = data_file[old_key]
