...
class ModuleContainer(torch.nn.Module):
    ...
    def __init__(self, observation_space: gym.Space, action_space: gym.Space, **kwargs) -> None:
        ...
        for container in self.CONTAINERS:
            ...
            module_class = vars(gamify.networks)[module_class] if isinstance(module_class, str) else module_class
...
class MultiEncoder(torch.nn.Module):
    def __init__(self, observation_space: gym.Space, action_space: gym.Space, **kwargs):
        ...
        flattened_observation_space = utils.flatten_dict(observation_space)
        assert all([k.replace("__", ".") in set(flattened_observation_space.keys()) for k in self.obs_keys])
        ...
        for k in self.obs_keys:
            ...
            module = module_class(flattened_observation_space[k.replace("__", ".")], action_space, **module_kwargs)
            modules[k] = module
        ...
        for k in self.obs_keys:
            ...
            if hasattr(module, "output_space"):
                ...
                output_shape = flattened_observation_space[k.replace("__", ".")].shape
    ...
    def forward(self, obs):
        return torch.cat(
            [self.encoders[k](utils.fetch_from_dict(obs, k.replace("__", "."))) for k in self.obs_keys], dim=-1
        )
