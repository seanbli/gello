...
class MinMaxActionNormalizer(Processor):
    ...
    def _denormalize(self, action) -> Dict:
        # Replace the action to be the correct
    ...
    def forward(self, batch: Dict):
        if isinstance(batch, dict):
            for k in ["action"]:
                if k in batch:
                    batch[k] = self._normalize(batch[k])
            return batch
        else:
            return self._normalize(batch)

    def unprocess(self, batch: Dict):
        if isinstance(batch, dict):
            for k in ["action"]:
                if k in batch:
                    batch[k] = self._denormalize(batch[k])
            return batch
        else:
            return self._denormalize(batch)
