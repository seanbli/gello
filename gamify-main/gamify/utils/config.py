...
def get_env(env: str, env_kwargs: Dict, wrapper: str, wrapper_kwargs: Dict) -> gym.Env:
    ...
    try:
        env = vars(gamify.envs)[env](**env_kwargs)
    except KeyError:
        env = gym.make(env, **env_kwargs)
    if wrapper is not None:
        env = vars(gamify.envs)[wrapper](env, **wrapper_kwargs)
    return env


class BareConfig(object):
    """
    This is a bare copy of the config that does not require importing any of the gamify packages.
...
class Config(BareConfig):
    ...
    def get_model(
        ...
        alg_class = vars(gamify.algs)[self["alg"]]
        dataset_class = None if self["dataset"] is None else vars(gamify.datasets)[self["dataset"]]
        validation_dataset_class = (
            None if self["validation_dataset"] is None else vars(gamify.datasets)[self["validation_dataset"]]
        )
        network_class = None if self["network"] is None else vars(gamify.networks)[self["network"]]
        optim_class = None if self["optim"] is None else vars(torch.optim)[self["optim"]]
        processor_class = None if self["processor"] is None else vars(gamify.processors)[self["processor"]]
