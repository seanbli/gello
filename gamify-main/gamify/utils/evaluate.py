...
def eval_diffusion_policy(
    ...
    assert history_length == 0, "History length != 0 has not been tested yet."
    ...
    for i in range(num_ep):
        ...
        if history_length > 0:
            obs = utils.unsqueeze(obs, 0)
            obs = utils.repeat_along_axis(obs, history_length, 0)
        if save_gif:
            frames.append(env.render(**render_kwargs))
        metric_tracker.reset()
        try:
            while not done:
                if robomimic:
                    for k in obs.keys():
                        if "image" in k:
                            obs[k] = np.flip(obs[k], axis=1).copy()
                batch = dict(obs=obs)
                if hasattr(env, "_max_episode_steps"):
                    batch["horizon"] = env._max_episode_steps - ep_length
                with torch.no_grad():
                    actions = model.predict(batch, **predict_kwargs)
                for action in actions[:horizon]:
                    ...
                    if robomimic:
                        ...
                        next_obs, reward, done, info = env.step(action, controller_type="OSC_POSE")
                    ep_reward += reward
                    metric_tracker.step(reward, info)
                    ep_length += 1
                    if save_gif and ep_length % every_n_frames == 0:
            ...
            print("[research] Caught KeyboardInterrupt")
            pass
        ...
        if save_gif:
            ...
            if robomimic:
                ...
                imageio.mimsave(os.path.join(path, gif_name), flipped_frames)
            else:
                imageio.mimsave(os.path.join(path, gif_name), frames)
