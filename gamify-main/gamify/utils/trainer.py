...
class Trainer(object):
    ...
    def train(self, path: str):
        ...
        print("[research] Parameter breakdown:")
        print(
            [
                (type(x).__name__, sum(p.numel() for p in x.parameters() if p.requires_grad))
                for x in self.model.network.children()
            ]
        )
        ...
        # Setup datasets
        print("[research] Setting up datasets.")
        ...
        system_metric_lists = defaultdict(list)
        ...
        if self.env is None or isinstance(self.env, EmptyEnv):
            env_freq = 10000000  # choose a really large value arbitrarily.
        ...
        while step <= self.total_steps:
            for batch in self.train_dataloader:
                ...
                if step % self.log_freq == 0:
                    ...
                    log_from_dict(logger, system_metric_lists, "system")
                    logger.dump(step=step, print_log=True)
                ...
                if self.checkpoint_freq is not None and step % self.checkpoint_freq == 0:
                    # Save a checkpoint
                    model_metadata = dict(step=step, epoch=epoch)
                    self.model.save(path, "temp_model", model_metadata)
                    shutil.move(f"{path}/temp_model.pt", f"{path}/model_{step}.pt")
