...
def shape(batch: Any):
    if isinstance(batch, dict):
        return {k: shape(v) for k, v in batch.items()}
    elif isinstance(batch, np.ndarray):
        return batch.shape
    elif isinstance(batch, list) or isinstance(batch, tuple):
        return [shape(v) for v in batch]
    elif isinstance(batch, torch.Tensor):
        return batch.shape
    elif isinstance(batch, h5py._hl.dataset.Dataset):
        return batch.shape
    elif isinstance(batch, h5py._hl.group.Group):
        return {k: shape(v) for k, v in batch.items()}
    elif isinstance(batch, h5py._hl.files.File):
        return {k: shape(v) for k, v in batch.items()}
...
def set_in_batch(batch: Any, value: Any, start: int, end: Optional[int] = None) -> None:
    if isinstance(batch, dict):
        ...
        if len(batch.shape) == len(value.shape) + 1 and batch.shape[-1] == 1:
            value = value[:, None]
        if end is None:
            batch[start] = value
        else:
...
def np_dataset_alloc(
    ...
    if isinstance(space, (dict, gym.spaces.Dict)):
        ...
        return np.empty((capacity, *begin_pad, *end_pad), dtype="<U128")
    else:
